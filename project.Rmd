...
ggplot(data = top_10_gdp_capita, mapping = aes(x = gdp_groups, y = suicide_per_capita, fill = gdp_groups))+
  geom_col()+
...
The visualization above divides countries by levels commonly used to analyze GDP per 
capita. The countries with GDP per capita 5,000-10,000 together experienced the highest 
number of suicides per 100k of any group. However, the group with the next highest suicide 
rate was those countries with GDP per capita of 2,000-5,000, followed by those with GDP 
per capita of 10,000-20,000, indicating a lack of a clear pattern between GDP per capita 
and suicides per 100k. This visualization possibly provides insight showing that countires 
with mid-to-low GDP per capita, averaged together, have the highest suicide rate. However, 
it further indicates that countries grouped by HDI is a more consistant and insighful 
visualization when compared with suicides per 100k. 
